...
RSpec.describe Profiler::SolrDocBuilder do
  ...
  context 'doc_hash' do
    before(:each) do
      ...
      @doc_hash = described_class.new(@fake_druid, @hdor_client, nil).doc_hash
    end
    ...
    it 'calls XmlSolrDocBuilder to populate hash fields from MODS' do
      ...
      described_class.new(@fake_druid, @hdor_client, nil).doc_hash
    end
  end
  ...
  context 'using Harvestdor::Client' do
    ...
    context 'smods_rec method (called in initialize method)' do
      it 'returns Stanford::Mods::Record object' do
        ...
        sdb = described_class.new(@fake_druid, @real_hdor_client, nil)
      ...
      it 'raises exception if MODS xml for the druid is empty' do
        ...
        expect { described_class.new(@fake_druid, @real_hdor_client, nil) }.to raise_error(RuntimeError, Regexp.new("^Empty MODS metadata for #{@fake_druid}: <"))
      end
      it 'raises exception if there is no MODS xml for the druid' do
        expect { described_class.new(@fake_druid, @real_hdor_client, nil) }.to raise_error(Harvestdor::Errors::MissingMods)
      end
    end
