...
RSpec.describe Profiler::XmlSolrDocBuilder do
  ...
  context 'doc_hash' do
    ...
    it 'has an entry for each top level element' do
      expect(@hash).to include(:e_e1_sim)
      expect(@hash).to include(:e_e2_sim)
    end
    it 'has an entry value for each occurrence of a repeated element' do
      expect(@hash).to include(e_e2_sim: %w(v2 v3))
    end
    it 'has an entry for the root element' do
      expect(@hash).to include(:e_sim)
    end
    it 'has entries for attributes on the root element' do
  ...
  context 'doc_hash_from_element' do
    ...
    context 'element children' do
      ...
      it 'has all attribute values across multiple children' do
        ...
        ng_el = ng_doc.root.xpath('/e').first
        expect(@xsdb.doc_hash_from_element(ng_el)).to include(e_e2_at2_sim: %w(a2 a3))
      end
    end
